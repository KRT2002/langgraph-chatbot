    ...
    try:
        ...
        if context_messages:
            base_classifier_messages.append(
                SystemMessage(content="Recent conversation context:")
            )
        ...
        base_classifier_messages.append(
            SystemMessage(content="You must not generate any explanation just respond with JSON array of relevant tool names:")
        )
        ...
        for attempt in range(max_retries):
            try:
                ...
                logger.info(f"Intent classifier selected {len(allowed_tool_names)} tools: {allowed_tool_names}")
        ...
        logger.warning(f"Intent classifier max retries ({max_retries}) exceeded, allowing all tools")
        return {"allowed_tools": [tool.name for tool in ALL_TOOLS]}
