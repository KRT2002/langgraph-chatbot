...
@tool
def file_operations(operation: str, filename: str, content: str = "") -> dict:
    """
    ...
    try:
        ...
        if operation == "read":
            ...
            return {"status": "success", "operation": "append", "filename": safe_filename}

        elif operation == "list":
            files = [f.name for f in SAFE_DIR.glob("*") if f.is_file()]
            logger.info(f"Listed {len(files)} files")
            return {"status": "success", "operation": "list", "files": files, "count": len(files)}
            ...
            return {"status": "success", "operation": "delete", "filename": safe_filename}
